...
static btVector3 getNormalizedVector2(const btVector3& v){
	btVector3 n(0, 0, 0);
...
*/
btVector3 KinematicCharacterController2::computeReflectionDirection(const btVector3& direction, const btVector3& normal){
...
*/
btVector3 KinematicCharacterController2::parallelComponent(const btVector3& direction, const btVector3& normal){
...
*/
btVector3 KinematicCharacterController2::perpindicularComponent(const btVector3& direction, const btVector3& normal){
	return direction - parallelComponent(direction, normal);
}

KinematicCharacterController2::KinematicCharacterController2(btPairCachingGhostObject* ghostObject, btConvexShape* convexShape, btScalar stepHeight, const btVector3& up){
...
KinematicCharacterController2::~KinematicCharacterController2(){
}

btPairCachingGhostObject* KinematicCharacterController2::getGhostObject(){
	return m_ghostObject;
}

bool KinematicCharacterController2::recoverFromPenetration(btCollisionWorld* collisionWorld){
...
void KinematicCharacterController2::stepUp(btCollisionWorld* world){
...
bool KinematicCharacterController2::needsCollision(const btCollisionObject* body0, const btCollisionObject* body1){
	bool collides = (body0->getBroadphaseHandle()->m_collisionFilterGroup & body1->getBroadphaseHandle()->m_collisionFilterMask) != 0;
	collides = collides && (body1->getBroadphaseHandle()->m_collisionFilterGroup & body0->getBroadphaseHandle()->m_collisionFilterMask);
	return collides;
}

void KinematicCharacterController2::updateTargetPositionBasedOnCollision(const btVector3& hitNormal, btScalar tangentMag, btScalar normalMag){
...
void KinematicCharacterController2::stepForwardAndStrafe(btCollisionWorld* collisionWorld, const btVector3& walkMove){
...
void KinematicCharacterController2::stepDown(btCollisionWorld* collisionWorld, btScalar dt){
	//std::cout << m_gravity << std::endl;
	...
	btScalar downVelocity = (m_gravity > 90.0) ? ((m_verticalVelocity < 0.f ? -m_verticalVelocity : 0.f) * dt) : 16.0f * dt;
	...
	while (1){
		...
		btScalar downVelocity2 = (m_gravity > 90.0) ? ((m_verticalVelocity < 0.f ? -m_verticalVelocity : 0.f) * dt) : downVelocity = 16.0f * dt;
		bool has_hit;
		if (bounce_fix == true)
...
void KinematicCharacterController2::setWalkDirection(const btVector3& walkDirection){
	m_useWalkDirection = true;
	m_walkDirection = walkDirection;
	m_normalizedDirection = getNormalizedVector2(m_walkDirection);
}

void KinematicCharacterController2::setVelocityForTimeInterval(const btVector3& velocity, btScalar timeInterval){
	//	printf("setVelocity!\n");
	//	printf("  interval: %f\n", timeInterval);
...
void KinematicCharacterController2::setAngularVelocity(const btVector3& velocity){
	m_AngVel = velocity;
}

const btVector3& KinematicCharacterController2::getAngularVelocity() const{
	return m_AngVel;
}

void KinematicCharacterController2::setLinearVelocity(const btVector3& velocity){
	m_walkDirection = velocity;
...
btVector3 KinematicCharacterController2::getLinearVelocity() const{
	return m_walkDirection + (m_verticalVelocity * m_up);
}

void KinematicCharacterController2::reset(btCollisionWorld* collisionWorld){
	m_verticalVelocity = 0.0;
	m_verticalOffset = 0.0;
	m_wasOnGround = false;
	m_wasJumping = false;
...
void KinematicCharacterController2::warp(const btVector3& origin){
	btTransform xform;
	xform.setIdentity();
...
void KinematicCharacterController2::preStep(btCollisionWorld* collisionWorld){
	m_currentPosition = m_ghostObject->getWorldTransform().getOrigin();
	m_targetPosition = m_currentPosition;
...
void KinematicCharacterController2::playerStep(btCollisionWorld* collisionWorld, btScalar dt){
	//	printf("playerStep(): ");
	//	printf("  dt = %f", dt);

	//m_gravity = m_safeGravity;
...
void KinematicCharacterController2::setFallSpeed(btScalar fallSpeed){
	m_fallSpeed = fallSpeed;
}

void KinematicCharacterController2::setJumpSpeed(btScalar jumpSpeed){
	m_jumpSpeed = jumpSpeed;
	m_SetjumpSpeed = m_jumpSpeed;
}

void KinematicCharacterController2::setMaxJumpHeight(btScalar maxJumpHeight){
	m_maxJumpHeight = maxJumpHeight;
}

bool KinematicCharacterController2::canJump() const{
	return onGround();
}

void KinematicCharacterController2::jump(const btVector3& v){
...
void KinematicCharacterController2::setGravity(const btVector3& gravity){
	if (gravity.length2() > 0) setUpVector(-gravity);

	m_gravity = gravity.length();
}

btVector3 KinematicCharacterController2::getGravity() const{
	return -m_gravity * m_up;
}

void KinematicCharacterController2::setMaxSlope(btScalar slopeRadians){
	m_maxSlopeRadians = slopeRadians;
	m_maxSlopeCosine = btCos(slopeRadians);
}

btScalar KinematicCharacterController2::getMaxSlope() const{
	return m_maxSlopeRadians;
}

void KinematicCharacterController2::setMaxPenetrationDepth(btScalar d){
	m_maxPenetrationDepth = d;
}

btScalar KinematicCharacterController2::getMaxPenetrationDepth() const{
	return m_maxPenetrationDepth;
}

bool KinematicCharacterController2::onGround() const{
	return (fabs(m_verticalVelocity) < SIMD_EPSILON) && (fabs(m_verticalOffset) < SIMD_EPSILON);
}

void KinematicCharacterController2::setStepHeight(btScalar h){
	m_stepHeight = h;
}

btVector3* KinematicCharacterController2::getUpAxisDirections(){
	static btVector3 sUpAxisDirection[3] = { btVector3(1.0f, 0.0f, 0.0f), btVector3(0.0f, 1.0f, 0.0f), btVector3(0.0f, 0.0f, 1.0f) };

	return sUpAxisDirection;
}

void KinematicCharacterController2::debugDraw(btIDebugDraw* debugDrawer){
}

void KinematicCharacterController2::setUpInterpolate(bool value){
	m_interpolateUp = value;
}

void KinematicCharacterController2::setUp(const btVector3& up){
...
void KinematicCharacterController2::setUpVector(const btVector3& up){
	if (m_up == up)
		return;
...
btQuaternion KinematicCharacterController2::getRotation(btVector3& v0, btVector3& v1) const{
