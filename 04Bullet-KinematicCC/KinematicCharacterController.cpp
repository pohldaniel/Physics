...
void KinematicCharacterController::stepDown(btCollisionWorld* collisionWorld, btScalar dt){
	...
	btScalar downVelocity = (m_gravity == 0.0) ? m_downVelocity * dt : ((m_verticalVelocity < 0.f ? -m_verticalVelocity : 0.f) * dt);
	...
	while (1){
		...
		btScalar downVelocity2 = (m_gravity == 0.0) ? m_downVelocity * dt : ((m_verticalVelocity < 0.f ? -m_verticalVelocity : 0.f) * dt);
...
void KinematicCharacterController::setVelocityForTimeInterval(const btVector3& velocity, btScalar timeInterval){
	//	printf("setVelocity!\n");
	//	printf("  interval: %f\n", timeInterval);
...
void KinematicCharacterController::warp(float posX, float posZ) {
	btTransform xform = m_ghostObject->getWorldTransform();
	xform.setOrigin(btVector3(posX, xform.getOrigin()[1], posZ));
	m_ghostObject->setWorldTransform(xform);
}
...
void KinematicCharacterController::setDownVelocity(btScalar downVelocity) {
	m_downVelocity = downVelocity;
}

btScalar KinematicCharacterController::getDownVelocity() const {
