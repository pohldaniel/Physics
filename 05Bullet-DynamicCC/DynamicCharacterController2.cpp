...
DynamicCharacterController2::DynamicCharacterController2(btRigidBody* rigidBody, btDiscreteDynamicsWorld* pPhysicsWorld, const Vector3f spawnPos, float radius, float height, float mass, float stepHeight, int collisionFilterGroup, int collisionFilterMask)
...
void DynamicCharacterController2::postStep()
{
	...
	RayResultCallback callback(m_pRigidBody);
	callback.m_collisionFilterGroup = m_pRigidBody->getBroadphaseHandle()->m_collisionFilterGroup;
	callback.m_collisionFilterMask = m_pRigidBody->getBroadphaseHandle()->m_collisionFilterMask;
	m_pPhysicsWorld->rayTest(from, to, callback);


	//IgnoreBodyAndGhostCast callback(m_pRigidBody, m_pGhostObject, mCollisionFilterGroup, mCollisionFilterMask);
	//m_pPhysicsWorld->rayTest(from, to, callback);
	...
	ConvexResultCallback callback(mRigidBody);
...
void DynamicCharacterController2::create(btMotionState* motionState, btDiscreteDynamicsWorld* physicsWorld, float mass, float radius, float height, int collisionFilterGroup, int collisionFilterMask, void* rigidBodyUserPointer) {
	mCollisionFilterGroup = collisionFilterGroup;
	mCollisionFilterMask = collisionFilterMask;
	...
	m_pRigidBody->setActivationState(DISABLE_DEACTIVATION);
	m_pRigidBody->setUserPointer(rigidBodyUserPointer);

	m_pRigidBody->setAngularFactor(ZERO_VECTOR); // No rotation allowed. We only rotate the visuals.
	m_pRigidBody->setSleepingThresholds(0.0, 0.0);
	physicsWorld->addRigidBody(m_pRigidBody, collisionFilterGroup, collisionFilterMask);


	
}

void DynamicCharacterController2::create(btRigidBody* rigidBody, btDiscreteDynamicsWorld* physicsWorld, int collisionFilterGroup, int collisionFilterMask, void* rigidBodyUserPointer) {
	...
	m_pRigidBody->setCollisionFlags(btCollisionObject::CF_DYNAMIC_OBJECT);
...
void DynamicCharacterController2::moveCharacterAlongY(float step) {
	btVector3 pos = m_pRigidBody->getWorldTransform().getOrigin();
	m_pRigidBody->getWorldTransform().setOrigin(pos + btVector3(0, step, 0));
}