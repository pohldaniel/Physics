...
void SkyBox::loadSide(unsigned int side_target, const char* fileName, bool flipVertical, bool flipHorizontal){

	int width, height, numCompontents;
	unsigned char* imageData = stbi_load(fileName, &width, &height, &numCompontents, 0);

	if (imageData != NULL) {

		if (flipVertical) {
			FlipVertical(width, height, numCompontents, imageData);
		}

		if (flipHorizontal) {
			FlipHorizontal(width, height, numCompontents, imageData);
		}

		glTexImage2D(side_target, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, imageData);
